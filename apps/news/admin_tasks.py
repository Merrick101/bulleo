"""
Admin config for managing periodic tasks related to news fetching.
...
# Unregister builtin celery-beat admins first
for m in (PeriodicTask, IntervalSchedule):
    try:
...
class CustomPeriodicTaskAdmin(PeriodicTaskAdmin):
    ...
    Custom admin for periodic tasks with:
      - Grouped panels instead of tabs
    ...
    fieldsets = (
        ...
        (
            "Advanced (args & execution)",
            {
                "classes": ("collapse",),
                "description": (
                    "If you need custom args or low-level AMQP tweaks, "
                    "expand this section."
                ),
                "fields": (
                    "args",
    ...
    change_form_template = (
        "admin/news/fetch_task_change_form.html"
    )
    list_display = (
        "name",
        "description_snippet",
    ...
    list_filter = ("enabled", "one_off", "interval")
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["help_text"] = format_html(
            "<div style='padding:10px; background:#f4f9ff; "
            "border-left:5px solid #007acc;'>"
            "<strong>Instructions:</strong><br>"
            "These are scheduled tasks that fetch articles based on "
            "their interval. Use the action menu to run manually."
            "</div>"
        )
    ...
    @admin.action(
        description="Run selected News Fetch Tasks now"
    )
    ...
    def run_now_view(self, request, object_id):
        ...
        if not task:
            self.message_user(
                request, "Task not found.", level=messages.ERROR
            )
            return redirect("..")
        try:
            current_app.send_task(task.task)
...
class CustomIntervalScheduleAdmin(IntervalScheduleAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["help_text"] = format_html(
            "<div style='padding:10px; background:#fff8e6; "
            "border-left:5px solid #ff9900;'>"
            "<strong>Instructions:</strong><br>"
            "These define how often news tasks repeat. Changing an "
            "interval will affect all tasks using it."
            "</div>"
        )
        return super().changelist_view(request, extra_context)
...
admin.site.register(
    PeriodicTask, CustomPeriodicTaskAdmin
)
admin.site.register(
    IntervalSchedule, CustomIntervalScheduleAdmin
)
